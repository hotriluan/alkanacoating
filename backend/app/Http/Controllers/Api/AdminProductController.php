...
class AdminProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $productData['order'] = is_numeric($order) ? (int)$order : 0;

            // Handle image upload (accept both 'image' and 'thumbnail')
            if ($request->hasFile('image') || $request->hasFile('thumbnail')) {
                $image = $request->file('image') ?: $request->file('thumbnail');
                $imageName = time() . '_' . Str::random(10) . '.' . $image->getClientOriginalExtension();
                $image->move(public_path('uploads/products'), $imageName);
                $productData['thumbnail'] = 'backend/public/uploads/products/' . $imageName;
            }

            $product = Product::create($productData);

            // Handle multiple image uploads (gallery) on create
            if ($request->hasFile('images')) {
                $images = $request->file('images');
                $primaryIndex = $request->get('primary_image_index', 0);
                foreach ($images as $index => $img) {
                    $imageName = time() . '_' . $index . '_' . Str::random(10) . '.' . $img->getClientOriginalExtension();
                    $img->move(public_path('uploads/products'), $imageName);
                    ProductImage::create([
                        'product_id' => $product->id,
                        'image_path' => 'backend/public/uploads/products/' . $imageName,
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Handle thumbnail replacement (accept both 'image' and 'thumbnail')
            if ($request->hasFile('image') || $request->hasFile('thumbnail')) {
                ...
                $imageName = time() . '_' . Str::random(10) . '.' . $image->getClientOriginalExtension();
                $image->move(public_path('uploads/products'), $imageName);
                $updateData['thumbnail'] = 'backend/public/uploads/products/' . $imageName;
            }
