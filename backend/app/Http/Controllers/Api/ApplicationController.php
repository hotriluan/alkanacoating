...
class ApplicationController extends Controller
{
        ...
        if ($request->hasFile('cv_file')) {
            ...
            if (!$cvPath) {
                return response()->json([
                    'message' => 'Lỗi server: Không thể lưu file CV. Vui lòng thử lại sau hoặc liên hệ quản trị viên.'
                ], 500);
            }
        }
    ...
    {
        if (!$application->cv_file || !Storage::disk('public')->exists($application->cv_file)) {
            return response()->json(['message' => 'Không tìm thấy file CV hoặc file đã bị xóa.'], 404);
        }

        return Storage::disk('public')->download($application->cv_file);
    }
