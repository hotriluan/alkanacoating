...
class PostController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Dynamically apply validation rules based on whether a file is provided for each field
        $rules['thumbnail'] = $request->hasFile('thumbnail') ?
            'nullable|image|mimes:jpeg,png,jpg,gif,webp|max:8192' :
            'nullable|string';
        ...
        // Handle image uploads to public/uploads/posts
        ...
        foreach (['thumbnail', 'image', 'featured_image'] as $field) {
            if ($request->hasFile($field)) {
                $file = $request->file($field);
                $name = time() . '_' . Str::random(10) . '.' . $file->getClientOriginalExtension();
                $file->move($uploadDir, $name);
                $validated[$field] = 'backend/public/uploads/posts/' . $name;
            }
        }
    ...
    public function update(Request $request, Post $post)
    {
        ...
        // If toggling to published but no published_at provided, set to now
        if (
            array_key_exists('is_published', $validated) && $validated['is_published'] &&
            (!array_key_exists('published_at', $validated) || empty($validated['published_at']))
        ) {
            $validated['published_at'] = now();
        }

        // Handle file uploads and replace old images when needed
        $uploadDir = public_path('uploads/posts');
        if (!is_dir($uploadDir)) {
            if (!mkdir($uploadDir, 0755, true)) {
                return response()->json([
        ...
        foreach (['thumbnail', 'image', 'featured_image'] as $field) {
            if ($request->hasFile($field)) {
                // delete old
                if ($post->{$field}) {
                    $this->deleteImageFile($post->{$field});
                }
                $file = $request->file($field);
                $name = time() . '_' . Str::random(10) . '.' . $file->getClientOriginalExtension();
                $file->move($uploadDir, $name);
                $validated[$field] = 'backend/public/uploads/posts/' . $name;
            } elseif (array_key_exists($field, $validated) && is_string($validated[$field]) && $validated[$field] === '') {
                // If empty string passed, treat as null (clear field)
