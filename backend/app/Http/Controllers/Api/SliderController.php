...
class SliderController extends Controller
{
    ...
    {
        $this->transformRequestKeys($request);
        ...
        // Sanitize certain fields to correct types
        if (isset($data['overlay_opacity'])) {
            $data['overlay_opacity'] = (int) $data['overlay_opacity'];
        }
        ...
        Log::debug('Slider store payload', ['data' => $data]);
    ...
    {
        // allow debugging the incoming request by logging instead of returning early
        $this->transformRequestKeys($request);
        ...
        // Sanitize types for consistency
        if (isset($data['overlay_opacity'])) {
            $data['overlay_opacity'] = (int) $data['overlay_opacity'];
        }
        if (isset($data['category_id']) && $data['category_id'] === '') {
        ...
        // Use fill + save to make logging clearer
        Log::debug('Slider update payload', ['id' => $slider->id, 'data' => $data]);
        $slider->fill($data);
        $slider->save();
    ...
    /**
     * Transforms incoming request keys to their correct counterparts.
     * This handles potential mismatches from the frontend.
     *
     * @param Request $request
     */
    private function transformRequestKeys(Request $request)
    {
        $transformationMap = [
            'text_nut_cta' => 'button_text',
            'vi_tri_text' => 'text_position',
            'do_toi_overlay' => 'overlay_opacity',
        ];

        $newData = [];
        foreach ($transformationMap as $from => $to) {
            if ($request->has($from)) {
                $newData[$to] = $request->input($from);
            }
        }
